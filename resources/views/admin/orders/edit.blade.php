...
@section('content')
<div class="card">
    <h2 style="margin:0 0 14px 0;">Uredi porudžbinu #{{ $order->id }}</h2>
    ...
    <form method="POST" action="{{ route('admin.orders.update', $order) }}" id="orderForm">
        ...
        <div class="card" style="background:#f9fafb; border:1px solid #e5e7eb; margin-bottom:14px;">
            <h3 style="margin:0 0 10px 0;">Podaci</h3>
                ...
                <div style="flex:3; min-width:300px;">
                    ...
                    <select name="client_id" required>
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" @selected($order->client_id == $client->id)>
        ...
        <div class="row" style="justify-content:space-between; gap:14px;">
            ...
            <div class="row" style="gap:10px;">
                <button class="btn" type="submit" id="saveBtn">Sačuvaj izmjene</button>
                <a class="btn btn-secondary" href="{{ route('admin.orders.index') }}">Nazad</a>
            </div>
        </div>
    </form>
</div>
...
(function () {
    ...
    function recalcTotal() {
        const total = items.reduce((sum, it) => sum + (parseFloat(it.line_total) || 0), 0);
    ...
    function render() {
        ...
        items.forEach((it, idx) => {
            ...
            inputsBox.insertAdjacentHTML('beforeend', `
                <input type="hidden" name="items[${idx}][description]" value="${escapeAttr(it.description)}">
